...
def vs_victory(main_char):
    main_char[5] += (main_char[5] * 0.05)  # 의용이 체력 5% 상승 / 최대체력
    main_char[2] += (main_char[2] * 0.05)  # 의용이 최소 공격력 5% 상승
    main_char[3] += (main_char[3] * 0.05)  # 의용이 최대 공격력 5% 상승
    main_char[1] += 1
    print(f"{main_char[0]}의 레벨:{main_char[1]}\n{main_char[0]}의 최대체력:{main_char[5]}\n"
          f"{main_char[0]}의 최소공격력:{main_char[2]}\nf{main_char[0]}의 최대공격력:{main_char[3]}")
    return main_char
...
def escape_process(escape_char, escape_enemy):
    ...
    if escape_rate(escape_enemy[1], escape_char[1]) == 1:  # 도망 성공
        ...
        print("도망가는데 실패했습니다.")
        uy = vs_attack(escape_enemy, escape_char)
...
def vs(ch_monster):
    ...
    elixir_usage = 0
    uy = ["초코의용😁", 1, 100, 150, 500, 500]  # 초코의용 리스트 이름/최소공격력/최대공격력/최소 체력/최대 체력
    potion = [100, 10]
    for i in range(500):
        ...
        while True:  # 싸움 반복문 열기
            ...
            if command == 1:
                ...
                if escape_process(uy, ch_monster) == 1:
                    break
                else:
                    pass


vs(monster_status(monster_meet()))