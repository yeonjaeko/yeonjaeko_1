...
def unannounced_inspection(worker, week_sum2):
    ...
    if worker <= 100:     # 아르바이트생이 100이하일때 발동
        ...
        week_sum2 = 0     # 한 주 종합 매출을 담기 위한 변수
        ...
        if c == 1:
          ...
          week_sum2 -= pm_unhappy_worker    # 1주 종합 매출에서 과태료를 빼줌
          print(f"{pm_unhappy_worker}원의 과태료를 냈습니다.")    # 과태료 프린트
          return week_sum2  # 과태료를 뺀 1주 종합 매출 리턴

        else:
         print("고용계약서에 아무런 문제가 없습니다.")
         return week_sum2   # 아무 문제 없는 1주 종합 매출 리턴
...
def week(Totalmoney): # Totalmoney = parameter(매개변수)
    ...
    b=sum(week_sum) # 아래 함수에 넣어주기 위해 변수 설정
    week_sum2 = unannounced_inspection(worker, b)  # 고용노동부 불시검문 함수 담아서 새로운 1주 매출 만듦 (연재.12.4)
    Net_profit += (sum(week_sum) - week_sum2)  # 고용노동부 불시검문 함수와 위크썸 매출을 뺀 값 == 정산값 (연재.12.4)
